...
def run() -> None:
    ...
    deployment_id = plugin_io.read_optional_str(payload, "deployment_id", f"{service}-{environment}-{version}")

    digest = hashlib.sha1(f"{deployment_id}:{service}:{environment}:{version}".encode()).hexdigest()

    plugin_io.success(
        {
            "external_ref": f"local:{service}:{environment}",
