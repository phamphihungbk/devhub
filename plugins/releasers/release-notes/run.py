...
def run() -> None:
    ...
    name = plugin_io.read_optional_str(payload, "name", tag)
    notes = plugin_io.read_optional_str(payload, "notes", f"Release {name}")
    repo_url = plugin_io.read_required_str(payload, "repo_url")

    digest = hashlib.sha1(f"{repo_url}:{target}:{tag}:{notes}".encode()).hexdigest()

    plugin_io.success(
        {
            "external_ref": f"release-notes:{tag}",
