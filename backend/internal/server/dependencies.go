...
func (s *Server) setupRouteDependencies(ctx context.Context, tracerProvider *sdktrace.TracerProvider, appLogger logger.Logger, dbConn *sqlx.DB) (httproute.Dependency, error) {
	...
	scaffoldRequestUsecase := scaffoldRequestUsecase.NewScaffoldRequestUsecase(s.cfg.App, dbApprovalRepo, localAIClient, localScaffoldSuggestionGenerator, dbPluginRepo, dbProjectRepo, dbScaffoldRequestRepo)
