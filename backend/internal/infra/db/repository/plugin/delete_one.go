...
func (r *pluginRepositoryImpl) DeleteOne(ctx context.Context, id uuid.UUID) (plugin *entity.Plugin, err error) {
	const errLocation = "[repository plugin/delete Delete] "
	defer misc.WrapErrorWithPrefix(errLocation, &err)

	pluginsTable := table.Plugins
	// SQL statement
	stmt := pluginsTable.DELETE().
	...
	if err != nil {
		...
		return nil, misc.WrapError(err, errs.NewDatabaseError("error while deleting plugin", err.Error()))
	}
